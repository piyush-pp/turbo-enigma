...
export const PublicBookingPage = () => {
  ...
  useEffect(() => {
    if (!selectedServiceId || !business) return
  ...
  const getActualCurrentStep = () => {
    ...
    if (!business?.isSingleStaff && !selectedStaffId) return 1
    if (!selectedSlot) return business?.isSingleStaff ? 1 : 2
    if (!formData.clientName || !formData.clientEmail) return business?.isSingleStaff ? 2 : 3
        ...
        <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full text-center">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6 text-left">
            <p className="text-sm text-gray-600 mb-2">
              <span className="font-semibold">Confirmation #:</span> {bookingId}
            </p>
            ...
            {selectedSlot && (
              <p className="text-sm text-gray-600">
                ...
                {format(new Date(selectedSlot.startTimeUtc), 'MMM d, yyyy HH:mm')}
              </p>
            )}
          </div>
          <button
  ...
  return (
    ...
    >
      ...
      {!business.isSingleStaff && actualStep === 1 && (
        <StaffSelectionStep
          staff={staff}
